...
export default function Home() {
  ...
  function add(product: Product) {
    ...
    if (variant.price === null) { setNotice("Contact Marc for current SLU-PP-332 pricing."); setTimeout(() => setNotice(""), 3500); return; }
    setCart((current) => {
      const found = current.find((item) => item.name === product.name && item.variant === variant.label);
      return found ? current.map((item) => item === found ? { ...item, quantity: item.quantity + 1 } : item) : [...current, { productId: productId(product), name: product.name, variant: variant.label, price: variant.price!, quantity: 1 }];
    });
  ...
  async function submitOrder(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/order", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ customer, researcher: gate, items: cart.map(({ productId, variant, quantity }) => ({ productId, variant, quantity })) }) });
    ...
    } catch { setNotice("We couldn't submit the order. Please try again."); }
  ...
  return <main>
    ...
    <header className="site-header"><a className="brand" href="#top"><span className="brand-symbol">CV</span><span>CLEAR VIEW<small>BIOLABS</small></span></a><nav><a href="#catalog">Catalog</a><a href="#standards">Standards</a><a href="/account">Account</a><a href="mailto:Marc@Clearviewbiolabs.com">Contact</a></nav><button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Open cart with ${count} items`}><span>Research Order</span><b>{count}</b></button></header>
