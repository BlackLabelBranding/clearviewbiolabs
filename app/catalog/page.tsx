...
export default function CatalogPage() {
  ...
  const [result, setResult] = useState<{ orderNumber: string; emailConfigured: boolean; subtotalCents: number; checkoutUrl: string } | null>(null);
  ...
  async function submitOrder(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await response.json() as { orderNumber?: string; emailConfigured?: boolean; subtotalCents?: number; checkoutUrl?: string; error?: string };
      if (!response.ok || !data.orderNumber || !data.checkoutUrl) {
        throw new Error(data.error || "Order could not be submitted");
      }
      const checkout = data as { orderNumber: string; emailConfigured: boolean; subtotalCents: number; checkoutUrl: string };
      setResult(checkout);
      setCart([]);
      window.location.assign(checkout.checkoutUrl);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "We couldn't submit the order. Please try again.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return <main>
    ...
    <header className="site-header"><Link className="brand" href="/"><span className="brand-symbol">CV</span><span>CLEAR VIEW<small>BIOLABS</small></span></Link><nav><Link href="/catalog">Catalog</Link><Link href="/account">Account</Link><Link href="/contact">Contact</Link></nav><button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Open cart with ${count} items`}><span>Research Order</span><b>{count}</b></button></header>
    ...
    <section className="order-explainer"><p className="eyebrow">HOW ORDERING WORKS</p><h2>Simple, direct, and secure.</h2><div><span><b>1</b><strong>Build your order</strong><small>Select the exact product, size, and quantity you need.</small></span><span><b>2</b><strong>Enter shipping details</strong><small>Your total is verified on our server before payment begins.</small></span><span><b>3</b><strong>Pay with USDC or USDT</strong><small>Complete payment through our secure, self-hosted PayRam checkout.</small></span></div></section>
    ...
    {checkoutOpen && <div className="modal-wrap"><button className="scrim" onClick={() => setCheckoutOpen(false)} aria-label="Close checkout" /><div className="checkout" role="dialog" aria-modal="true" aria-labelledby="checkout-title"><button className="modal-close" onClick={() => setCheckoutOpen(false)}>×</button>{result ? <div className="success"><span>✓</span><p className="eyebrow">PAYMENT READY</p><h2>Your secure checkout is ready.</h2><p>Order <strong>{result.orderNumber}</strong></p><div><b>Pay with USDC or USDT</b><p>Continue to PayRam to choose your network and complete payment. Product ships only after the payment is confirmed on-chain.</p></div><a className="gold-button" href={result.checkoutUrl}>Continue to Secure Payment</a></div> : <><div className="checkout-head"><p className="eyebrow">SECURE CHECKOUT</p><h2 id="checkout-title">Where should we send your research materials?</h2><p>After your shipping details are saved, you will continue to our self-hosted PayRam checkout to pay with USDC or USDT.</p></div><form onSubmit={submitOrder}><div className="checkout-grid"><label>First name<input required name="firstName" defaultValue={gate.firstName} /></label><label>Last name<input required name="lastName" defaultValue={gate.lastName} /></label><label className="wide">Email address<input required name="email" type="email" defaultValue={gate.email} /></label><label className="wide">Shipping address<input required name="address" autoComplete="street-address" /></label><label>City<input required name="city" /></label><label>State<input required name="state" /></label><label>ZIP code<input required name="zip" inputMode="numeric" /></label><label>Phone<input required name="phone" type="tel" /></label></div><label className="check"><input required type="checkbox" /> <span>I confirm this order is exclusively for lawful research use and that the shipping information is accurate.</span></label><div className="checkout-total"><span><p>Order subtotal</p><strong>{money(subtotal)}</strong></span><button className="gold-button" disabled={submitting}>{submitting ? "Creating secure checkout…" : "Continue to USDC / USDT Payment"}</button></div></form></>}</div></div>}
  </main>;
}
